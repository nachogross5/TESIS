...
#      Todo el resto del algoritmo se construyo sobre esa esquirla.
#      FIX: el filtro de borde ya NO puede descartar la masa osea dominante.
#      La isla de referencia se elige entre las de densidad osea IGNORANDO el
#      criterio de borde, y se marca como "referencia protegida" en la tabla.
#      El veto por borde se aplica solo a las islas que NO son la referencia.
#
# [P2] TC 559 - MemoryError, nunca generaba el craneo.
#      Sintoma: "Unable to allocate 1.71 GiB for an array with shape
#      (3, 584, 512, 512)".
...
#      V5 lo llamaba UNA VEZ POR RONDA dentro del while. Ademas V5 guardaba
#      una mascara booleana de volumen completo por isla: 31 islas x 153 MB
#      = 4.7 GB. No es un caso raro: es que el algoritmo escala mal.
#      FIX: no se usa EDT de volumen completo. Las distancias entre islas se
#      calculan con scipy.spatial.cKDTree sobre los voxeles de SUPERFICIE de
#      cada isla (dos ordenes de magnitud menos puntos), y la matriz de
#      distancias se calcula UNA sola vez, no una por ronda. Ademas no se
#      guarda una mascara por isla: se trabaja sobre un unico array de
#      etiquetas (ndimage.label), que es ~306 MB en vez de 4.7 GB.
#
# [P3] TC 403 - "Craneo_Final" se comia la mandibula.
#      Evidencia en la consola de V5:
#        "Limpieza de malla en 'Craneo_Final': 454 regiones -> 1 conservadas
#         (umbral 10% del tamano mayor)."
#      CAUSA: enviar_a_planner() de V5 limpiaba la malla con un umbral
#      RELATIVO al 10% del tamano de la region mayor, y conservaba UNA sola
#      region de 454. La mandibula es una region conexa aparte, mas chica que
#      el 10% de la boveda -> se borraba. Lo mismo le pasaria a cualquier
#      placa craneal desconectada por una sutura abierta.
#      FIX: umbral ABSOLUTO en cantidad de puntos (MINIMO_PUNTOS_MALLA),
#      calibrado al ruido de marching cubes y no al tamano de la boveda. Se
#      informa el tamano de cada region descartada para poder auditarlo.
#
# [P4] TC 403 - la almohadilla no se eliminaba entera.
#      CAUSA (dos, sumadas):
#       (a) La espuma tiene densidad irregular: al umbralar en 300 HU queda
#           partida en varios componentes conexos. Los que alcanzan el borde
#           se descartan; los que quedan flotando en el medio no tocan
#           ninguna cara del array y pasan el filtro.
#       (b) Una TC no reconstruye todo el array: reconstruye un CIRCULO
#           inscripto, y rellena el resto con un valor fijo. Un objeto
#           cortado por ese borde circular NUNCA llega a una cara plana del
#           array, asi que el test de las 6 caras no lo ve.
#      FIX: (a) filtro de densidad por percentil 95 de HU: la espuma y el
#           plastico entran raspando los 300 HU y se quedan en 350-550,
#           mientras el hueso cortical supera los 700 incluso en pediatricos
#           poco mineralizados. Es un criterio fisico, independiente del
#           encuadre y de la posicion.
#           (b) el "exterior" se detecta leyendo el valor de relleno en las
#           esquinas de un corte central; asi el borde CIRCULAR del FOV y las
#           caras planas del array se testean con el mismo criterio.
#      Se agrega ademas un detector de losa (tabla de la camilla): isla que
#      abarca >= FRACCION_LOSA_FOV del campo en algun eje y mide menos de
#      ESPESOR_LOSA_MM en su eje mas fino.
#
# [P5] Confusion Craneo_Automatico / Craneo_Final.
#      FIX: nombres explicitos y un unico resultado final. La segmentacion se
#      llama "CranioPlan_Segmentacion"; el segmento fusionado "Craneo_Final";
#      el modelo exportado "Craneo_Final_Modelo". Se imprime cual es cual.
#
# [P6] Filtro de tamano relativo (5% de la isla mayor).
#      Una calota de lactante ronda los 100-120 cm3 de hueso: el 5% son 5-6
#      cm3, que es el orden de magnitud de una mandibula entera. En tu propia
#      corrida del 403 aparecian islas de 7.44, 5.28 y 5.09 cm3, es decir
#      justo en el filo del corte. Se reemplaza por un piso ABSOLUTO en cm3.
#
# [P7] TC de JUAN ETCHEVARNE - la almohadilla quedaba como referencia.
#      Sintoma: isla de referencia = 79.49 cm3, p95=1609 HU, tocando el borde;
#      el craneo real (58.74 cm3, p95=1429) quedaba segundo.
#      CAUSA: dos cosas a la vez.
#       (a) En ESE equipo el soporte de inmovilizacion NO es espuma blanda:
#           su p95 es 1609 HU, igual de denso que hueso cortical. La losa de
#           la camilla daba 1417 y otra pieza 2288. El filtro de densidad
#           solo sirvio para los fragmentos chicos (438 y 427 HU). O sea: la
#           densidad NO es un discriminador universal, depende del equipo.
#       (b) La primera version de v6 hacia la referencia SIEMPRE inmune al
#           veto de borde, para arreglar la TC 175. Eso resuelve un caso y
#           rompe el opuesto.
...
#      FIX: dos pasadas, con un criterio que decide cual de los dos casos es.
#       Pasada 1: veto de borde para TODAS las islas, sin excepcion (la
#                 logica de V5, que es la que funciona en Juan).
#       Chequeo : se suma el volumen oseo aceptado.
#       Pasada 2: SOLO si la pasada 1 no llega a VOLUMEN_CRANEO_MINIMO_CM3,
#                 se repite dejando a la referencia exenta del veto, y se
#                 avisa por consola.
#      En Juan la pasada 1 acepta ~71 cm3 -> no hay pasada 2 y la almohadilla
#      se descarta por borde. En la TC 175 la pasada 1 acepta ~5 cm3 -> se
#      dispara el rescate y el craneo recortado vuelve a ser la referencia.
#
# ------------------------------------------------------------
...
#   la mandibula. Se eliminan solas SOLO si el FOV las recorta (caso 403). Si
#   entran completas, van a aparecer en la revision y hay que borrarlas a
#   mano. Un criterio anatomico (recorte por debajo del cráneo) es posible
#   pero es una decision clinica: conviene preguntarle a Marito si al cirujano
#   le molesta verlas o si prefiere tenerlas como referencia de orientacion.
# - ALMOHADILLA CON INSERTO METALICO: pasaria densidad y losa. Hay que
#   borrarla a mano.
# - LA DENSIDAD NO ES UNIVERSAL. Confirmado en la TC de Juan: el soporte de
#   inmovilizacion de ese equipo tiene p95=1609 HU. El filtro de densidad
#   sirve para espuma y plastico blando, no para equipamiento denso. Quien
#   resuelve esos casos es el veto de borde. Los dos filtros son
#   complementarios y ninguno reemplaza al otro.
# - UMBRAL_DENSIDAD_HU = 500 esta elegido a priori. Si en un lactante de
#   pocos meses el hueso esta muy poco mineralizado podria descartar hueso
#   real. Salvaguarda: si el filtro deja el pool vacio se desactiva solo y
#   avisa. Igual hay que mirar la columna p95 de la tabla en los casos
#   nuevos antes de darlo por bueno.
# - Las distancias entre islas usan como maximo MAX_PUNTOS_KDTREE puntos de
#   superficie por isla (submuestreo por paso fijo). Sobre una superficie
#   craneal el error de la distancia minima queda muy por debajo del
#   milimetro, despreciable frente al margen de 40 mm.
# ============================================================
...
# Tambien se puede cambiar despues con usar_volumen("...") y volver a
# correr generar().
NOMBRE_VOLUMEN = None
...
# Piso ABSOLUTO de volumen para que una isla llegue a la revision.
# Reemplaza al umbral relativo del 5% de V5 (ver [P6]).
VOL_MINIMO_CM3 = 0.5
...
# Es una cota anatomica, no un parametro de ajuste: la boveda de un recien
# nacido ya supera holgadamente este valor. Solo se usa para decidir si la
# pasada con veto de borde estricto dio un resultado plausible o si hay que
# recurrir al rescate (ver [P7]). Se compara contra el TOTAL aceptado, no
# contra una isla suelta, para no romperse cuando las suturas abiertas
# parten la calota en varias placas.
VOLUMEN_CRANEO_MINIMO_CM3 = 20.0

# Filtro de densidad (ver [P4]). Percentil 95 de HU dentro de la isla.
# Por debajo de UMBRAL_DENSIDAD_HU se descarta como material no oseo.
# Entre ese valor y UMBRAL_DENSIDAD_DUDOSA_HU se conserva pero se marca
# REVISAR en la tabla.
...
CRANIOPLAN_BLOQUE_A = "v6 (2026-07-31) consola"
...
def _valorDeRelleno(volArr):
    ...
    reconstruccion. Se lee en las cuatro esquinas de un corte central: si las
    cuatro coinciden y son muy negativas, ese es el relleno.

    Devuelve el valor, o None si no se puede determinar (en cuyo caso solo se
    usan las caras planas del array como borde).
    """
    z = volArr.shape[0] // 2
    corte = volArr[z]
...
def _mascaraExterior(volArr, valorRelleno):
    ...
    reconstruccion mas las seis caras planas del array. Testear contra esto
    cubre de una sola vez el borde CIRCULAR del FOV (que el test de caras
    planas de V5 no veia) y el borde del array.
    """
...
def _etiquetasTocandoExterior(labels, exterior):
    """
    Etiquetas con al menos un voxel dentro del exterior o pegado a el
    (6-adyacencia). Se resuelve con seis desplazamientos en vez de dilatar el
    volumen completo: la dilatacion pediria otro array de volumen entero y en
    estudios grandes es justamente lo que hacia fallar a V5 por memoria.
    """
    tocan = set()
...
def _puntosDeSuperficie(labels, etiqueta, caja, espaciadoZYX, maxPuntos):
    ...
    Se trabaja dentro de la caja envolvente, no sobre el volumen completo. Se
    usan solo los voxeles de superficie porque la distancia minima entre dos
    solidos se alcanza siempre en sus superficies: da el mismo resultado que
    la EDT de volumen completo con dos ordenes de magnitud menos de puntos.
    """
...
def analizarVolumen(volArr, espaciadoZYX, cfg):
    ...
    for k in range(1, nIslas + 1):
        ...
        # las motitas ya estan descartadas y no vale la pena el costo. En la
        # TC de Juan esto evita 7919 percentiles inutiles.
        p95 = None
...
def seleccionarCandidatas(labels, filas, espaciadoZYX, cfg,
    ...
    exentarReferenciaDelBorde=False
        El veto de borde vale para TODAS las islas, sin excepcion. Es la
        logica de V5. Resuelve el caso de Juan, donde la almohadilla es mas
        grande y mas densa que el craneo pero toca el borde y el craneo no.

    exentarReferenciaDelBorde=True
        La referencia puede tocar el borde. Resuelve el caso de la TC 175,
        donde el FOV recorta el craneo y lo unico que no toca el borde es una
        esquirla de 2.84 cm3. Modo de rescate: solo se usa si la pasada
        estricta no llega a un volumen oseo plausible.
...
def crecerPorProximidad(labels, filas, referencia, espaciadoZYX, cfg, cache):
    ...
    Reemplaza al distance_transform_edt de V5, que reservaba un array
    (3, Z, Y, X) int32 -por eso el MemoryError de la TC 559- y ademas se
    recalculaba en cada ronda. Aca las distancias se calculan con cKDTree
    sobre los voxeles de SUPERFICIE de cada isla, y la matriz se arma una
    sola vez. El cache de puntos y arboles se comparte entre las dos pasadas
    de seleccionarCandidatas(), asi el rescate no cuesta el doble.
    """
...
def listar_volumenes():
    ...
    Sirve para elegir a mano la serie mientras el Paso 1 del modulo (la
    seleccion automatica de serie) se sigue ajustando."""
...
def generar():
    ...
    segNode = slicer.mrmlScene.AddNewNodeByClass(
        'vtkMRMLSegmentationNode', 'CranioPlan_Segmentacion')
...
def confirmar_craneo():
    """
    Fusiona las piezas que quedaron en un unico segmento "Craneo_Final".
    ...
    representacion de superficie vieja de cada segmento. Aca la superficie se
    rehace desde cero con RemoveClosedSurfaceRepresentation() antes de
    CreateClosedSurfaceRepresentation(), porque este ultimo NO reconvierte si
    la representacion ya existe.
    """
    segNode = _estado.get("segNode")
    volumeNode = _estado.get("volumeNode")
    ...
    segmentacion.GetSegment(base).SetName("Craneo_Final")
    ...
    print("CranioPlan: listo. Segmento 'Craneo_Final' dentro del nodo "
          "'CranioPlan_Segmentacion'. Ahora: enviar_a_planner()")
    return True
...
def _limpiarRuidoMalla(polyData, minimoPuntos):
    ...
    abierta que mida menos del 10% de la boveda se borra sola. El umbral
    absoluto esta calibrado al ruido de la conversion, no al tamano del
    craneo.
    """
    if polyData is None or polyData.GetNumberOfPoints() == 0:
        return polyData
...
def enviar_a_planner():
    """
    Exporta 'Craneo_Final' a un modelo listo para el planificador de cortes.
    ...
    if not segId:
        print("CranioPlan: todavia no existe 'Craneo_Final'. Corre confirmar_craneo().")
        return None
    ...
    modelo = slicer.mrmlScene.AddNewNodeByClass(
        'vtkMRMLModelNode', slicer.mrmlScene.GenerateUniqueName("Craneo_Final_Modelo"))
...
def ayuda():
    ...
    print("  usar_volumen(i)       -> elige serie por indice, nombre o fragmento")
    print("  generar()             -> corre el Bloque A sobre la serie elegida")
    ...
    print("  confirmar_craneo()    -> fusiona lo que quedo en 'Craneo_Final'")
    print("  enviar_a_planner()    -> exporta 'Craneo_Final_Modelo'")
    print("")
    print("Nodos: segmentacion = 'CranioPlan_Segmentacion' (segmento "
          "'Craneo_Final' tras confirmar)")
    print("       modelo       = 'Craneo_Final_Modelo' (lo que usa el planner)")
