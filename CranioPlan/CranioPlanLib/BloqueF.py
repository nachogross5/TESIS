...
#
#   (a) TEST 3D DE BURBUJA. El hueco tiene que ser un ESPACIO ABIERTO: aire
#       que sobrevive a una erosion de UMBRAL_ESPACIO_ABIERTO_MM. Un poro o un
#       espacio medular del diploe no sobrevive; la cavidad craneal, la orbita
#       y el exterior si. Mide el hueco EN VOLUMEN, no a lo largo de una
#       linea, y por eso es dificil de enganar.
#
#   (b) MARGEN A LO LARGO DEL RAYO. Ademas, desde ese punto tiene que haber
#       MARGEN_SALIDA_MM de aire continuo. Confirma que no caimos en una
#       burbuja.
#
...
# ############################################################
# Un rayo que no atraviesa se CORTA EN SECO a TOPE_SIN_SALIDA_FACTOR veces el
# espesor tipico de esa misma curva. No puede hacer dano lejos del sitio. Y el
# punto queda marcado en rojo.
#
# ############################################################
# QUE SE CONSERVA DE VERSIONES ANTERIORES
# ############################################################
...
from .Comun import (NOMBRE_MODELO_CRANEO, NOMBRE_NODO_SEGMENTACION,
                    NOMBRE_SEGMENTO_CRANEO, NOMBRE_SEG_TRABAJO,
                    NOMBRE_CARPETA_SH, NOMBRE_MODELO_PICOS,
                    NOMBRE_FID_SIN_PASAR)

# Alias con el nombre que usaba el script suelto, para que el codigo de abajo
# sea el mismo que Nacho tiene en su version de consola.
...
#   real siempre cae en un numero impar de voxeles; cortar() informa cual salio.
...
UMBRAL_ESPACIO_ABIERTO_MM = 3.0 # test 3D de burbuja. Subir a 4-5 si el corte
#   frena dentro de hueso muy poroso. Bajar a 2 si se pasa de largo hacia
#   estructuras vecinas.
MARGEN_SALIDA_MM       = 2.0    # aire continuo a lo largo del rayo desde el
#   punto de salida. Confirmacion de que no caimos en una burbuja de hueso.
...
#   espesor tipico de la misma curva. Subir a 2-3 solo si se sabe que ahi el
#   hueso es genuinamente mas grueso que en el resto de la curva.
...
# --- Higiene de escena ---
PREFIJO_CURVAS         = ""     # el modulo lo pone en "Corte_" para no
#   agarrar curvas viejas que hayan quedado de otra sesion.
...
def configurar(**kwargs):
    """
    Cambia constantes de configuracion desde el modulo.

    Se hace asi, y no con un objeto de configuracion pasado por parametro, a
    proposito: el archivo tiene que seguir siendo pegable tal cual en la
    consola de Slicer para que Nacho pueda probar un cambio sin abrir el
    modulo. Si las constantes pasaran a ser atributos de una clase, esa
    propiedad se pierde.

    Uso tipico desde el widget:  configurar(GROSOR_CORTE_MM=1.5, log=self.log)
    """
    for clave, valor in kwargs.items():
        if clave == "log":
            _SALIDA["log"] = valor if valor is not None else print
        elif clave in globals():
            globals()[clave] = valor
        else:
            _p("Bloque F: no existe la opcion '%s', se ignora." % clave)
...
class CampoNormales:
    ...
    hueso donde se dibujo la curva: ni la tabla interna, ni los poros, ni la
    ...
    def _construir_adyacencia(self):
        ...
        if data.size % 4 != 0 or not np.all(data[0::4] == 3):
            _p("  AVISO: la malla no es puramente de triangulos; "
...
def _resamplear_puntos(puntosVtk, distanciaMM):
    """Re-distribuye la polilinea densa de Slicer en pasos parejos. Tiene que
    ser fino porque la cortina se estampa punto por punto."""
...
def _direcciones_de_curva(campo, posiciones, esExtension, esCerrada, distMuestreo):
    """Normal de superficie por punto + orientacion de signos con guarda +
    suavizado. Un solo punto malo NO puede invertir el signo del resto."""
    n = len(posiciones)
    crudas = [None] * n
    ...
    if esCerrada and n > 2 and float(np.dot(N[-1], N[0])) < 0.0:
        circular = False
        _p("    AVISO: el lazo no cierra con signo consistente; "
    ...
    norm[norm < 1e-12] = 1.0
    S = S / norm[:, None]
    return S, {"n": n, "sospechosos": contador["sospechosos"],
...
def _indice_de_salida(esHueso, esAbierto, g, mMargen):
    ...
    Se acepta la salida solo si se cumplen las DOS condiciones:
      (a) la muestra esta en ESPACIO ABIERTO (test 3D de burbuja);
      (b) desde ahi hay mMargen muestras seguidas SIN hueso.
...
# ============================================================
# PARTE 5 - TAPONES (solo para lazos cerrados)
# ============================================================
def _tapon_de_curva(curvaNode, longitud):
    ...
    try:
        ...
        _p("  no pude generar el tapon de '%s': %s" % (curvaNode.GetName(), e))
        return None
...
def _mascaras_de_tapones(volumeNode, longitud, curvasCerradas):
    ...
    for curva in curvasCerradas:
        ...
        mNode = slicer.mrmlScene.AddNewNodeByClass(
            "vtkMRMLModelNode", "_Tapon_%s" % curva.GetName())
    ...
    for segId, nombreCurva in mapaNombre.items():
        try:
            ...
            _p("  no pude rasterizar el tapon de '%s': %s" % (nombreCurva, e))
...
def _limpiar_malla(modelNode, umbral=UMBRAL_LIMPIEZA_MALLA):
    ...
    _p("    limpieza de malla: %d regiones -> %d" % (nRegiones, len(conservar)))
...
def _reportar_curvas(cerradas, abiertas):
    _p("")
    _p("--- CURVAS QUE SE VAN A USAR ---")
    if PREFIJO_CURVAS:
        _p("  (solo las que empiezan con '%s')" % PREFIJO_CURVAS)
    for c in cerradas:
        _p("  [lazo ] %s  (%d puntos)" % (c.GetName(), c.GetNumberOfControlPoints()))
    for c in abiertas:
        _p("  [linea] %s  (%d puntos)" % (c.GetName(), c.GetNumberOfControlPoints()))
    _p("  TOTAL: %d" % (len(cerradas) + len(abiertas)))
...
def _preparar_entradas():
    try:
        ...
        _p("ERROR: no encuentro la segmentacion '%s'. Falta generar el craneo."
           % NOMBRE_SEGMENTACION)
        return None
    try:
        craneoModel = slicer.util.getNode(NOMBRE_MODELO_CRANEO)
    except Exception:
        _p("ERROR: no encuentro el modelo '%s'. Falta confirmar el craneo."
           % NOMBRE_MODELO_CRANEO)
        return None
    if not craneoModel.IsA("vtkMRMLModelNode"):
        _p("ERROR: '%s' no es un modelo 3D." % NOMBRE_MODELO_CRANEO)
        return None
    volumeNode = _volumen_de_referencia(segOrigen)
    if volumeNode is None:
        _p("ERROR: no encuentro el volumen de la tomografia.")
        return None
    segIdHueso = segOrigen.GetSegmentation().GetSegmentIdBySegmentName(
        NOMBRE_SEGMENTO_CRANEO)
    if not segIdHueso:
        _p("ERROR: no hay segmento '%s'. Falta confirmar el craneo."
           % NOMBRE_SEGMENTO_CRANEO)
        return None
    cerradas, abiertas = _curvas_de_la_escena()
    if not cerradas and not abiertas:
        _p("ERROR: no hay ninguna linea de corte trazada.")
        return None
...
def _reportar_curva(nombre, tipo, diag):
    ...
    _p("      continuidad: separacion max entre rayos %.2fmm -> x%d rayos"
       % (diag.get('separacionMaxima', 0), diag.get('factorSupersampleo', 1)))
    if diag.get("separacionTope"):
        _p("      >> el supersampleo llego al tope: subir MAX_FACTOR_SUPERSAMPLEO.")
    if diag.get("sospechosos", 0) > 0:
        _p("      %d salto(s) de direccion frenados por la guarda"
           % diag["sospechosos"])
    if "separo" in diag:
        if diag["separo"]:
...
def diagnosticar():
    ...
    O sea: mirando los picos se ve literalmente hasta donde llega el corte y
    donde se queda corto, ANTES de cortar nada.

    Devuelve un dict con el resumen por curva, para la interfaz.
    """
    ...
    _p("Preparando el volumen (mascara de espacio abierto)...")
    vol = VolumenHueso(boneMask, volumeNode)
    _p("Salida del hueso = espacio abierto > %smm Y %smm de margen libre"
    ...
    nNoTotal = _marcar_puntos(np.vstack(sinPasar) if sinPasar else None,
                              NOMBRE_FID_SIN_PASAR, (1.0, 0.0, 0.0))
    _p("")
    _p("AZUL = el corte atraviesa de lado a lado.  ROJO = se queda adentro.")
    _p("No se corto nada.")
    return {"curvas": resumen, "puntosRojos": nNoTotal}
...
def cortar():
    """
    Ejecuta los cortes y deja los fragmentos en la carpeta
    'CranioPlan_Fragmentos' del panel Data, que es de donde los lee el
    Bloque G.

    Devuelve un dict con el resumen, o None si no se pudo cortar.
    """
    ent = _preparar_entradas()
    if ent is None:
        return None
    ...
    _p("  sin salida -> corte en seco a %sx el espesor tipico (min %smm)"
       % (TOPE_SIN_SALIDA_FACTOR, TOPE_SIN_SALIDA_MIN_MM))
    _p("Piezas de hueso antes del corte: %d" % nPadres)
    ...
    for curva in curvas:
        ...
        if anillo is None:
            _p("  ADVERTENCIA: no se pudo construir el corte de '%s': %s"
               % (curva.GetName(), diag.get('error', 'motivo desconocido')))
            resumenCurvas.append({"nombre": curva.GetName(),
                                  "error": diag.get("error", "desconocido")})
            continue
        anilloTotal |= anillo
    ...
    if not anilloTotal.any():
        _p("ERROR: no se pudo construir ningun corte.")
        slicer.mrmlScene.RemoveNode(segTrabajo)
        return None

    if kerfReal:
        _p("")
        _p("Ancho REAL de la ranura: %.2fmm (%d voxeles)"
           % (kerfReal, int(round(kerfReal / min(vol.spacing)))))

    nRojos = _marcar_puntos(np.vstack(sinPasar) if sinPasar else None,
                            NOMBRE_FID_SIN_PASAR, (1.0, 0.0, 0.0))
    if nRojos:
        _p("%d punto(s) que NO atraviesan, marcados en rojo." % nRojos)
    if noSepararon:
        _p("AVISO: %d corte(s) NO separaron el hueso: %s"
           % (len(noSepararon), noSepararon))

    boneCut = np.logical_and(boneMask, np.logical_not(anilloTotal))
    _p("")
    _p("Corte total restado: %d voxeles (%.3fcm3)."
       % (int(np.count_nonzero(anilloTotal)),
          np.count_nonzero(anilloTotal) * voxel_mm3 / 1000.0))

    etiquetasHijas, nHijas = ndimage.label(boneCut)
    _p("Piezas tras el corte: %d" % nHijas)
    ...
    for madre, labs in hijasDeMadre.items():
        ...
        for lab in labs:
            ...
            if curva is not None:
                ...
                piezas.append((mask, "Tapa_%s" % curva, True, curva, vol_cm3))
            elif creadaPorCorte and madreTieneTapa[madre]:
                contadorResto += 1
                piezas.append((mask, "Resto_%d" % contadorResto, False, None, vol_cm3))
            elif creadaPorCorte:
                contadorFragmento += 1
                piezas.append((mask, "Fragmento_%d" % contadorFragmento,
                               False, None, vol_cm3))
            else:
                if vol_cm3 < VOLUMEN_MINIMO_CM3:
                    continue
                contadorHueso += 1
                piezas.append((mask, "Hueso_%d" % contadorHueso, False, None, vol_cm3))

    if not piezas:
        _p("ERROR: no sobrevivio ningun fragmento.")
        slicer.mrmlScene.RemoveNode(segTrabajo)
        return None
    ...
    for mask, nombre, esTapa, curva, v in piezas:
        if esTapa and curva in sufijos:
            nombre = "%s_%s" % (nombre, chr(ord('a') + sufijos[curva]))
            sufijos[curva] += 1
    ...
    for mask, nombre, esTapa, curva, v in piezasFinales:
        ...
        _p("  %s [%s]: vol=%.4fcm3 (%d cavidad(es) rellenada(s))"
           % (nombre, 'TAPA' if esTapa else 'resto/hueso', volf, nRell))

    segTrabajo.GetSegmentation().RemoveSegment(idHueso)
    nombresTapas = set(n for _, n, e, _, _ in piezasFinales if e)

    segTrabajo.GetSegmentation().SetConversionParameter("Smoothing factor",
                                                        SUAVIZADO_EXPORT)
    ...
    if not slicer.vtkSlicerSegmentationsModuleLogic.ExportSegmentsToModels(
        ...
        _p("Fallo la exportacion a modelos. Revisar el Error log de Slicer.")
        return None

    _p("")
    _p("--- CONTROL DE CALIDAD ---")
    nodoPorNombre = {}
    ...
    for nombre in idsPorNombre.keys():
        ...
        if not cand:
            cand = [m for m in modelos if m.GetName() == nombre]
        if not cand:
            _p("  ADVERTENCIA: no encontre el modelo '%s'." % nombre)
            continue
        node = cand[0]
        ...
        _p("  %s [%s]: %s  vol=%.4fcm3"
           % (nombre, 'tapa de hueso' if nombre in nombresTapas else 'resto/hueso',
              'WATERTIGHT (apto STL)' if cerrada else 'ABIERTA (NO apta para STL)',
              volumen / 1000.0))
        detallePiezas.append({"nombre": nombre, "watertight": bool(cerrada),
                              "volumenCM3": volumen / 1000.0})
    ...
    for curva, items in itemsPorCurva.items():
        sub = _subcarpeta(shNode, folderItemId, "Tapas_%s" % curva)
        for itemId in items:
    ...
    _p("")
    _p("Listo: %d pieza(s), %d watertight, %d tapa(s) de hueso."
       % (total, listos, len(nombresTapas)))
    _p("Estan en la carpeta '%s' del panel Data." % NOMBRE_CARPETA_SH)

    return {
        "piezas": detallePiezas,
        "total": total,
        "watertight": listos,
        "curvas": resumenCurvas,
        "noSepararon": noSepararon,
        "puntosRojos": nRojos,
        "kerfReal": kerfReal,
        "piezasAntes": nPadres,
    }
...
def exportar_stl(carpeta_destino):
    ...
    for node in nodos:
        ...
        if not cerrada:
            _p("  %s: NO watertight, no se exporta." % node.GetName())
            continue
        ruta = os.path.join(carpeta_destino, node.GetName() + ".stl")
        slicer.util.saveNode(node, ruta)
        _p("  %s -> %s" % (node.GetName(), ruta))
        n += 1
    _p("%d STL exportado(s)." % n)
    return n
...
def limpiar():
    ...
    for nombre in (NOMBRE_SEG_TRABAJO, NOMBRE_MODELO_PICOS, NOMBRE_FID_SIN_PASAR):
        ...
        while nodo is not None:
            ...
            nodo = slicer.mrmlScene.GetFirstNodeByName(nombre)
    _p("Nodos de trabajo del corte borrados.")


def ocultarPicos(visible=False):
    """Prende o apaga la visualizacion de picos del diagnostico."""
    for nombre in (NOMBRE_MODELO_PICOS, NOMBRE_FID_SIN_PASAR):
        nodo = slicer.mrmlScene.GetFirstNodeByName(nombre)
