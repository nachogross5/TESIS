# ============================================================
# CRANIOPLAN - BLOQUE F v9 (UNIFICADO)
#
...
def cortar():
    ...
    print(f"Curvas de corte: {[c.GetName() for c in curvas]}")
    ...
    for curva in curvas:
        anillo = _anillo_de_corte(boneMask, curva, mallaHueso, volumeNode, esCerrada=True)
        if anillo is None:
            print(f"  ADVERTENCIA: no se pudo construir el anillo de '{curva.GetName()}'.")
            continue
        anilloTotal |= anillo
        print(f"  anillo de '{curva.GetName()}': {int(np.count_nonzero(anillo))} voxeles")
    ...
    for madre, labs in hijasDeMadre.items():
        ...
        for lab in labs:
            ...
            if curva is not None:
                # TAPA: se conserva SIEMPRE, sin umbral de tamano
    ...
    for mask, nombre, esTapa, curva, vol in piezas:
        ...
        piezasFinales.append((mask, nombre, esTapa, vol))

    # ---- 9. Materializar cada pieza como segmento, rellenar cavidades ----
    idsPorNombre = {}
    for mask, nombre, esTapa, vol in piezasFinales:
    ...
    nombresTapas = set(n for _, n, esT, _ in piezasFinales if esT)
    ...
    for i in range(hijos.GetNumberOfIds()):
        ...
        tipo = "tapa de hueso" if nombre in nombresTapas else "resto"
    ...
    # Releer los hijos ahora que los modelos existen.
    hijos = vtk.vtkIdList()
    shNode.GetItemChildren(folderItemId, hijos)
    itemsPorCurva = {}
    for i in range(hijos.GetNumberOfIds()):
        ...
        if node is None or not node.IsA("vtkMRMLModelNode"):
            continue
        nombre = node.GetName()
        if nombre.startswith("Tapa_"):
            # Tapa_<curva> o Tapa_<curva>_a: la curva es lo del medio.
            resto = nombre[len("Tapa_"):]
            # sacar sufijo _a/_b/_c si existe
    ...
    for curva, items in itemsPorCurva.items():
        # Solo crear subcarpeta si esa curva genero mas de una tapa,
        # o siempre, segun preferencia. Aca: siempre, para orden.
...
print("Bloque F v9 (unificado) cargado.")
