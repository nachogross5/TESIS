...
def centrar_vista():
    """
    Encuadra la vista 3D (y las 2D) sobre lo que hay en escena.

    Es lo mismo que el boton "Center view" de la barra de la vista 3D. Hace
    falta porque Slicer NO reajusta la camara solo cuando aparecen nodos
    nuevos: la deja donde estaba, y si el craneo cae fuera de ese encuadre la
    vista se ve vacia aunque la segmentacion exista.

    Se llama solo al final de generar(); tambien se puede correr a mano.
    """
    try:
    ...
    try:
        for i in range(lm.threeDViewCount):
            ...
            v = w.threeDView()
            try:
                v.resetFocalPoint()          # equivale a "Center view"
            except Exception:
                pass
            try:
                ren = v.renderWindow().GetRenderers().GetFirstRenderer()
                if ren is not None:
                    ren.ResetCamera()        # ajusta ademas el zoom
            except Exception:
                pass
            try:
                v.forceRender()
...
def ayuda():
    ...
    print("  resaltar(n) / mostrar_todas() / centrar_vista()")
